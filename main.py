...
def creatCfgFile():
    ...
    if(os.path.exists("Genshin.cfg")):
        ...
        for strLine in strLines:
            if(strLine.split("Speed=")[0].strip()=="" and int(strLine.strip().split("Speed=")[1]) in range(1,6)):
                SpdLevel = int(strLine.split("Speed=")[1].strip())
                continue
            if(strLine.split("log=")[0].strip()=="" and int(strLine.strip().split("log=")[1]) in range(0,2)):
